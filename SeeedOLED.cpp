...
void SeeedOLED::setXY(unsigned char Page, unsigned char Column)
// set absolute XY, 0,0 top L,  to 8,128 lower R
{
    sendCommand(0xB0 + Page); 
    sendCommand(0x00 + (Column & 0x0F)); 	//set column lower address
    sendCommand(0x10 + ((Column>>4)&0x0F)); 	//set column higher address
}
...
void SeeedOLED::putCharInverted(unsigned char C)
{
    if(C < 32 || C > 127)
    {
    C=' '; //Space
    ...
    {
       //read bytes from code memory
       inv = ~pgm_read_byte(&BasicFont[C-32][i]);
       sendData(inv);
    }
}
...
void SeeedOLED::putStringInverted(const char *String)
{
    unsigned char i=0;
    while(String[i])
    {
        putCharInverted(String[i]);     
        i++;
    }
}
